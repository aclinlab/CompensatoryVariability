...
while(~constraints)
    ...
    theta = theta + eta_theta * C_theta .* errorInActivity;
    theta(theta<0) = 0;
    ...
    Y_incInh = A - APLgain .* sum(A,1) - C_theta .* theta;
    CL_incInh = mean(Y_incInh(:)>0);
    Y_incInh(Y_incInh<0) = 0; % crucial before getting mean activity
...
MBmodel.modelType = 'homeostatic spike threshold aka MAGENTA model';
