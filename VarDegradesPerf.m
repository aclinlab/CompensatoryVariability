...
spmd
   ...
   %% allow alpha to be a free parameter: achieving the 2nd spar. constraint CL=10%            
           ...
           thetaH_Ftheta=(C_thetaHF*thetaH_Ftheta);
               ...
               Clevels=[CLevelP{1},CLevelP{2},CLevelP{3},CLevelP{4}];
               INHAbs_CL=[INHAbs_CLP{1},INHAbs_CLP{2},INHAbs_CLP{3},INHAbs_CLP{4}];
...
if(odors==100)
 save(strcat(['VarDegradesPerf_fly_wNoise',num2str(randomTrials),num2str(noiseScale)]) , 'thisW_HomogModel','APLgains',...
...
% if(odors==110)
% save(strcat(['VarDegradesPerf_HOInp_fly_wNoise',num2str(randomTrials),num2str(noiseScale)]) , 'thisW_HomogModel','APLgains',...
% 'thetaH_Ftheta', 'thisW','thetaS_Ftheta' ,'thisW_varWonly','thisW_varN','theta_varw_fixedN_scalar','theta_varN_fixedW_scalar',...
% 'thetaH', 'thetaS','theta_varw_fixedN', 'theta_varN_fixedw','PNtrials','classAction1' );
% 
% end
...
% calculate KCs dimensionality and lifetime sparsity when 
% K (num of odors)=100 and at the least noise scale 
if ( (odors==100) && (noiseScale==1))
    
    % Calculating the models dimension: the main extreme ends (Homogenous and
    % random models, black & red respectively)
    dim_S(randomTrials)= dimInputCurrent(Calc_C(Y));
    dim_HF(randomTrials)=dimInputCurrent(Calc_C(YHomog_Ftheta));

    % Calculate Lifetime Sparsity.
    % lifeTime Sparseness, Andrew Lin et.al. 2014
    ...
    % lifetime sparsity= [N_KCs X random_Flies] matrix, 2000x20
    lifeTime_Spar(:,randomTrials)= Spar;
    H_lifeTime_Spar(:,randomTrials)= HomogenousSpar;

    % standard deviation in the Lifetime Spar values, exclduing NAns (silent
    % KCs)
...
%                 alpha= 0.001*(10^(l_r));
       ...
       %% for each odour cloud, calculate its separability from other odors clouds using Davis-Bouldin Index (DBI).
       % measuring separability of the KCs representation in the 8 models
...
% KCs dimensionality in the random and homogenous models
save('dim_S.mat','dim_S');
save('dim_HF.mat','dim_HF');

% KCs lifetime sparsity and standard dev. in their lifetime spar. values
save('H_lifeTime_Spar.mat','H_lifeTime_Spar');
save('lifeTime_Spar.mat','lifeTime_Spar');
save('std_H_LTSpar.mat','H_LTSpar');
save('std_S_LTSpar.mat','S_LTSpar');

% angular distance between odor pairs
save('S_PW_AngDist.mat','S_PW_AngDist');
save('H_PW_AngDist.mat','H_PW_AngDist');

% angular distance between 'good' and 'bad' odors classes
save('S_Cw_AngDist.mat', 'S_Cw_AngDist');
save('HF_Cw_AngDist.mat','HF_Cw_AngDist');
...
% DBI between 'good' and 'bad' odors classes
save('S_Cw_DBI.mat','S_Cw_DBI');
save('HF_Cw_DBI.mat','HF_Cw_DBI');

% DBI between odor pairs
save('S_DBi_odoursPairs.mat','S_DBi_odoursPairs');
save('HF_DBi_odoursPairs.mat','HF_DBi_odoursPairs')
