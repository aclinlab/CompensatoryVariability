classdef neurSkel
    properties
        ...
        % fields of synSet (stands for "synapse set"):
            % if P = # of unique partners, S = # of uniqiue synapses, L = #
            % of unique synapse locations
            % uniquePartners: P elements, holds the body_Ids of all unique partners in
            %   this synSet
            % uniquePartnerIndices: S elements, range from 1 to P
            % synLocs: L x 3 elements - xyz locations
            % synLinks: L elements - skeleton link indices of each location
            % synGroupings: L cell array: the unique synapses in each
            %   synapse locatation (range from 1 to S)
            % rawData: the original data read in from the .csv file
            % synROIs
            % synROIkey
            % synLocsOfOrigSyn: S elements, the synapse location of each
            %   original synapse, range form 1 to L
    ...
    methods
        ...
        function obj = getNodes(obj)
            ...
            % get the ROIs of each node
            [obj.ROIskey,~,obj.nodeROIs] = unique(obj.skel.roi);
            disp('creating links...');
            for i=1:numNodes
                if mod(i,10000)==0
                    i
            ...
            % get the ROI of each link
            % ignore links that have 0 values
            linkIndices = 1:size(obj.links,1);
            nonzeroLinkIndices = linkIndices(all(obj.links,2));
            obj.linkROIs = zeros(size(obj.links,1),1);
            for i=nonzeroLinkIndices
                % if the ROIs of the two end points differ, then
                % arbitrarily assign it to the maximum index value
                obj.linkROIs(i) = max(obj.nodeROIs(obj.links(i,:)));
            end
            toc
        end
...
%             % THIS FUNCTION IS DEPRECATED, LEAVING IT HERE FOR LEGACY
%             % PURPOSES. WE ARE ADVISED NOT TO USES ROI MESHES
%             % ROI is a mesh
